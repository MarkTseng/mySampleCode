...
-- Created:     16/11/2001
-- RCS-ID:      $Id: scribble.wx.lua,v 1.25 2008/01/22 04:45:39 jrl1 Exp $
...
ID_PENCOLOUR   = wx.wxID_HIGHEST + 2
ID_PENWIDTH    = wx.wxID_HIGHEST + 3
ID_PENSTYLE    = wx.wxID_HIGHEST + 4
ID_PENWIDTH_SPINCTRL = wx.wxID_HIGHEST + 5
...
function main()
    ...
    -- Styles really only work for long lines, when you change direction the styles
    ...
    penWidthSpinCtrl = wx.wxSpinCtrl(toolBar, ID_PENWIDTH_SPINCTRL, "3",
    ...
    frame:Connect(wx.wxID_NEW, wx.wxEVT_COMMAND_MENU_SELECTED,
            function (event)
                ...
                repeat
                    ...
                    if (#s == 0) then
                        return -- they canceled the dialog
                    end
                    ...
                    if (w == nil) or (h == nil) or (w < 2) or (h < 2) or (w > 10000) or (h > 10000) then
                        wx.wxMessageBox("Please enter two positive numbers for the width and height separated by a space",
