...
function DumpBindingInfo(binding)
    ...
    if true then
        ...
        local keys = { "name", "wxluamethods", "wxluamethods_n", "classInfo", "wxluatype", "baseclassName", "baseclass", "enums", "enums_n" }
...
function CreatewxLuaBindClass(tbl)
    local t = {
        ...
        ["col_labels"] = { "Class Name", "# Methods", "wxClassInfo", "Class Tag", "Base Class Name", "# Enums" },
        ["object_type"] = "wxLuaBindClass"
    }

    -- items in table from binding.GetClassArray are these
    -- { "name", "wxluamethods", "wxluamethods_n", "classInfo", "wxluatype", "baseclassName", "baseclass", "enums", "enums_n" }
    ...
    --{ "name"[data], "wxluamethods_n", "classInfo", "wxluatype", "baseclassName"[data], "enums_n"[data] }
    for i = 1, #tbl do
        local item = {
            tbl[i].name,
        ...
        if (type(tbl[i].baseclass) == "userdata") then
            item.col_icons[5] = list_images.folder
            item.data[5] = tbl[i].baseclass
        end
        ...
        if (tbl[i].baseclassName) and (type(tbl[i].baseclass) ~= "userdata") then
...
function CreatewxLuaBindMethod(tbl, classname)
    local t = {
        {"..", ["icon"] = list_images.folder},
        ["col_labels"] = { "name", "method_type", "wxluacfuncs_n", "basemethod", "minargs", "maxargs", "argtype_names", "argtypes" },
    ...
    for i = 1, #tbl do
        local class_name = ""
        if tbl[i].class_name then class_name = tbl[i].class_name.."::" end

        local item = {
            class_name..tbl[i].name,
            CreatewxLuaMethod_TypeString(tbl[i].method_type),
            tbl[i].wxluacfuncs_n,
            "",
            "",
            "",
        ...
        if tbl[i].classname then
            item[1] = tbl[i].classname.."::"..item[1]
        end

        -- This method has a basemethod and can be expanded
        if type(tbl[i].basemethod) == "userdata" then
            item[4] = tbl[i].basemethod.class_name
            item.data[4] = tbl[i].basemethod
            item.col_icons[4] = list_images.folder
        end

        -- Add in the CFuncs
        item.data[1] = tbl[i].wxluacfuncs
        item.color = wx.wxBLUE
        table.insert(t, item)

        -- keys for CFunc = { "lua_cfunc", "type", "minargs", "maxargs", "argtype_names", "argtypes" }
        local cfunc_t = CreatewxLuaBindCFunc(tbl[i].wxluacfuncs)
        for j = 2, #cfunc_t do
            local cft = {item[1].." func "..j-1, cfunc_t[j][2], "", "", cfunc_t[j][3], cfunc_t[j][4], cfunc_t[j][5], cfunc_t[j][6]}
            if string.find(cfunc_t[j][2], "Overload", 1, 1) then cft.color = list_colors.green end
            table.insert(t, cft)
        end
    end
...
function OnListItemActivated(event)
    ...
    elseif (data_index > 1) and data.object_type == "wxLuaBindClass" then
        ...
        if (col == 0) and (type(data[data_index].data[1]) == "table") then
            ...
            if frame:GetMenuBar():IsChecked(ID_VIEW_BASECLASS_FUNCTIONS) then
                local c = data[data_index].data[5]
                while type(c) == "userdata" do
        ...
        elseif (col == 4) and (type(data[data_index].data[col+1]) == "userdata") then
    ...
    elseif (data_index > 1) and data.object_type == "wxLuaBindMethod" then
        ...
        if (col == 0) and (type(data[data_index].data[col+1]) == "table") then
            t = CreatewxLuaBindCFunc(data[data_index].data[col+1])
            t.class_name = data.class_name
        elseif (col == 3) and (type(data[data_index].data[col+1]) == "userdata") then
