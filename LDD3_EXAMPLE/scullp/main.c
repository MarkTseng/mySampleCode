...
static ssize_t scullp_defer_op(int write, struct kiocb *iocb, const struct iovec *iov,
			   unsigned long nr_segs, loff_t pos)
{
	...
	for (i = 0; i < nr_segs; i++) {
  		if (write) {
                	result = scullp_write (iocb->ki_filp, iov[i].iov_base, iov[i].iov_len, &pos);
		} else { /* read */
                	result = scullp_read (iocb->ki_filp, iov[i].iov_base, iov[i].iov_len, &pos);
                }
                if (result < 0)
                      	return result;
		len += result;                                 
