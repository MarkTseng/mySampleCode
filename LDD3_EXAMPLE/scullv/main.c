...
static ssize_t scullv_defer_op(int write, struct kiocb *iocb, const struct iovec *iov,
			   unsigned long nr_segs, loff_t pos)
{
	...
	for (i = 0; i < nr_segs; i++) {
  		if (write) {
                	result = scullv_write (iocb->ki_filp, iov[i].iov_base, iov[i].iov_len, &pos);
		} else { /* read */
                	result = scullv_read (iocb->ki_filp, iov[i].iov_base, iov[i].iov_len, &pos);
                }
                if (result < 0)
                      	return result;
		len += result;                                 
...
static ssize_t scullv_aio_read(struct kiocb *iocb,
...
static ssize_t scullv_aio_write(struct kiocb *iocb,
			       const struct iovec *iovec,
			       unsigned long nr_segs,
		loff_t pos)
{
