...
static ssize_t sculld_defer_op(int write, struct kiocb *iocb, const struct iovec *iov,
			   unsigned long nr_segs, loff_t pos)
{
	...
	for (i = 0; i < nr_segs; i++) {
  		if (write) {
                	result = sculld_write (iocb->ki_filp, iov[i].iov_base, iov[i].iov_len, &pos);
		} else { /* read */
                	result = sculld_read (iocb->ki_filp, iov[i].iov_base, iov[i].iov_len, &pos);
                }
                if (result < 0)
                      	return result;
		len += result;                                 
...
static int sculld_register_dev(struct sculld_dev *dev, int index)
{
	...
	if ((result = device_create_file(&dev->ldev.dev, &dev_attr_dev))) {
...
int sculld_init(void)
{
	 ...
	 */
	result = register_ldd_driver(&sculld_driver);
	if (result)
		goto fail_malloc;
	...
	for (i = 0; i < sculld_devs; i++) {
		...
		if (sculld_register_dev(sculld_devices + i, i))
	...
	/* Consider modifying sculld_cleanup() so that it can be called here,
           to avoid the following duplicate code. */
