/* aldtest.c
 *
 * Create by Brad Nichols. 10/15/93
 *
 ...
 * a equivelent amount of "work" being done in each case. The 
...
char **argv;
{
  ...
  if (argc >= 2)  {
	...
	((rtn = strcmp(argv[1], "help")) == 0)) {
      ...
      printf("\t\t\taccess to its accounts. Each access to an account\n");
  ...
  for (i = 0; i < num_clnts; i++) {
    ...
    if ((child_pids[i] = fork()) == 0) {
      ...
      DPRINTF(("LOAD_TEST, Child %d: I'm alive\n", i));
	...
	fprintf(stderr,
		"atm_test.c, atm_stress_routine returned failure\n");
	exit(1); 
      }
  ...
  diff_nsecs = (signed long) (etime.tv_nsec - stime.tv_nsec);

  DPRINTF(("\nLOAD_TEST, diff_secs %ld, diff_nsecs %ld\n",
	   diff_secs, diff_nsecs)); 

  elapsed_time_sec = (double)diff_secs + (double)diff_nsecs*1E-9;
