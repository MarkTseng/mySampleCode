 ...
 * atm_srv_cancel.c
 *
 * Variant using pthread cancellation for shutdown of 
 * multi-threaded server for the ATM example.
...
pthread_t   *worker_threadp;   /* a pointer to a newly created thread 
				   ...
				   need to have a handle to anyother thread.
...
void *process_request(void *input_orderp)
{
  ...
  /* Defered cancellation is enabled, arrange cleanup */
  pthread_cleanup_push(free, input_orderp);
  ...
  pthread_testcancel(); /* the transaction routine may have disabled
			   cancellation by this point, but it won't
			   hurt anything to check it either way */

  server_comm_send_response(workorderp->conn, resp_buf);

  pthread_cleanup_pop(1); /* free the workorder buffer */
...
void deposit(char *req_buf, char *resp_buf)
{
...
#elif defined(SINGLE_DATABASE_MUTEX_SYNC)
  pthread_cleanup_push(pthread_mutex_unlock, (void *)&global_data_mutex);
  pthread_mutex_lock(&global_data_mutex);
#elif defined(ACCOUNT_MUTEX_SYNC)
  pthread_cleanup_push(pthread_mutex_unlock, (void *)&account_mutex[id]);
  ...
  /* Add new balance */
    ...
    /*
     * Check if we have been cancelled, then disallow 
     * cancellation once we start writing the database
     */
    pthread_testcancel();
