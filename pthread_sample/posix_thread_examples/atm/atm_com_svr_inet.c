...
void server_comm_init(void)
{
  ...
  if ((srv_sock[0] = socket (AF_INET, SOCK_STREAM, 0)) < 0) 
    perror("creating socket"), exit(1);
  ...
  if ((bind(srv_sock[0], (struct sockaddr *)&sock_name, sizeof(struct sockaddr_in))) < 0) 
    perror("binding socket to a name"), exit(1);
...
int handle_new_request(fd_set read_selects, int **connp, char **req_bufp)
{
  ...
  if ((br = read(srv_sock[i], 
    ...
    if (br == 0) {
      ...
      perror("Read() error on get request"), exit (1);
    } else {
      fprintf(stderr, 
