 ...
 * atm_svr_processes.c
 *
 * Multi-process server program for the Automated Teller 
 * Machine (ATM) example.
 *
...
/* Semaphores per set and total semaphores per host can be system limited. 
...
#if defined(__digital__) && defined(__unix__)
#else /* GCC/Linux */
/* cleaner wrapper for _sysctl() used in man page example */
...
int get_sems_per_set(void)
{
  int rtn;
#if defined(__digital__) && defined(__unix__)
  int   sps;
  if ((rtn = table(TBL_SEMINFO, SEMINFO_MSL, &sps, 1, sizeof(int)) < 1)) {
	perror("table() call for sems per set");
	exit(1);
  }
  return sps;
#else /* GCC/Linux */
  int   semparams[100];
  int   semparams_lth;
  int   sysctl_name[] = { CTL_KERN, KERN_SEM };

  semparams_lth=sizeof(semparams);
  if (rtn = sysctl(sysctl_name, sizeof(sysctl_name)/sizeof(sysctl_name[0]), semparams, &semparams_lth, 0, 0)) {
    perror("SERVER main(): sysctl query for sems per set failed\n");
    exit(1);
  }
  return semparams[0];
#endif
...
void atm_server_init(int argc, char **argv)
{
  ...
  if ((sems_per_set = get_sems_per_set()) < 0) {
    perror("SERVER main(): get_sems_per_set failed\n");
    exit(1);
  }
...
void open_account(char *resp_buf)
{
  ...
  if (semop(single_database_sem_id, &single_database_sem_buf, 1) < 0) {
  ...
  if (semop(open_account_sem_id, &open_account_sem_buf, 1) < 0) {
...
void deposit(char *req_buf, char *resp_buf)
{
  ...
  if (semop(account_sem_array[id].set_id, &account_sem_buf, 1) < 0) {
