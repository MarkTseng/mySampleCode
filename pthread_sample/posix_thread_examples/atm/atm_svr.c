...
/* Database Account Access Synchronization Options, uncomment on of the 
   following or specify on the command line.  ACCOUNT_MUTEX_SYNC is the
   default. */
...
#ifndef NO_DATABASE_SYNC
#ifndef SINGLE_DATABASE_MUTEX_SYNC
#define ACCOUNT_MUTEX_SYNC
#endif
#endif
...
main(int argc, char **argv)
{
  pthread_t   *worker_threadp;   /* a pointer to a newly created thread 
				   object.  We don't need to keep the 
				   thread handle around because we don't
				   ever use it after detaching the thread,
				   so we just reuse this pointer */
  ...
  DPRINTF(("SERVER: main(), shutting down\n"));
