 ...
 * atm_svr_tpool.c
 *
 * Variant using thread pool for efficient thread management
 * of Multi-Threaded Server program for the Automated Teller 
...
static tpool_t atm_thread_pool;
...
void atm_server_init(int argc, char **argv)
{
  ...
  tpool_init(&atm_thread_pool, ATM_MAX_THREADS, ATM_MAX_THREADS, 0);
