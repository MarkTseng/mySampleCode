...
void * BGET_T:: bget(bufsize requested_size)
...
void *BGET_T::bgetz(bufsize size)
//  bufsize size;
{
...
void *BGET_T::bgetr(void *buf, bufsize size)
//  void *buf;
//  bufsize size;
{
    void *nbuf;
...
void BGET_T::bectl(compact, acquire, release, pool_incr)
...
void BGET_T::bpool(void *buf, bufsize len)
//  void *buf;
// bufsize len;
{
...
void BGET_T::bstats(bufsize *curalloc, bufsize* totfree,bufsize*  maxfree, long * nget, long *nrel)
...
void BGET_T::bstatse(bufsize * pool_incr, long * npool,long* npget, long * nprel, long * ndget, long * ndrel)
//  bufsize *pool_incr;
...
void BGET_T::bpoold(void *buf,int dumpalloc, int dumpfree)
//  void *buf;
...
void BGET_T::srand(unsigned int seed)
//  unsigned int seed;
{
	next = seed;
}
...
int BGET_T::  bcompact(bufsize bsize, int seq)
//  bufsize bsize;
//  int seq;
{
#ifdef CompactTries
...
void *BGET_T::bexpand(bufsize size)
//  bufsize size;
{
    void *np = NULL;
    bufsize cural, totfree, maxfree;
...
bufsize BGET_T::blimit(bufsize bs)
//  bufsize bs;
{
    if (bs < (bufsize)sizeof(char *)) {
	bs = sizeof(char *);
    }
...
#else
    bp = (char *)malloc(PoolSize);
    assert(bp != NULL);
    bpool((void *) bp, (bufsize) PoolSize);
#endif

    stats((char*)"Create pool");
    V bpoolv((void *) bp);
    bpoold((void *) bp, dumpAlloc, dumpFree);

    for (i = 0; i < TestProg; i++) {
	char *cb;
	bufsize bs = (long)pow(x, (double) (rand() & (ExpIncr - 1)));
	...
	*((char **) cb) = (char *) bchain;
	...
	if ((rand() & 0x20) == 0) {
		...
		if (fb != NULL) {
		    ...
		    bs = (long)pow(x, (double) (rand() & (ExpIncr - 1)));
		    bs = blimit(bs);
#ifdef BECtl
...
/**************************************************
 *
 * bget interface based on bget class
 * alex,2010-11
 *
 *************************************************/
#include "bget-api.h"
...
#ifdef API_DEBUG
#include <stdio.h>
#define bget_log(format, args...) printf(format, args)
#else
#define bget_log(format, args...)
#endif

struct bget_pool_info_t
{
    int cnt;
    BGET_T *ptab[BGET_MAX_POOL_NO];
};

/* define bget_pool_info_t bpool_info access apis*/
#define __bget_get_pool(handle) (bpool_info.ptab[(handle)])
#define __bget_pool_cnt (bpool_info.cnt)
static struct bget_pool_info_t bpool_info={0};
...
bget_handle_t bget_create_pool(void *buffer, bget_bufsize_t len)
{
    ...
    //bget_log("in bget_create_pool, buffer = 0x%x, len = %d\n", buffer, len);

    /* init only once */
    if (__bget_pool_cnt == 0) // not safe in muti-thread
        memset(&bpool_info, 0, sizeof (bpool_info));

    /* bget_pool_cnt is the next free index*/
    if (__bget_pool_cnt >= BGET_MAX_POOL_NO) // bpool_info.pool_cnt indidata free handle index
    {
        bget_log("create pool err, pool cnt: %d\n", __bget_pool_cnt);
        return BGET_FAIL;
    }


    /* init pool*/
    handle = (bget_handle_t)__bget_pool_cnt;
    __bget_get_pool(handle) = new BGET_T;
    __bget_get_pool(handle)->bpool(buffer, len);
    __bget_pool_cnt++; // not safe in muti-thread
...
{
	if (__bget_get_pool(handle)) {
		delete __bget_get_pool(handle);
	}
}

void *bget_get(bget_handle_t handle, bget_bufsize_t size) // alloc buffer from pool indicated by handle
{
    return __bget_get_pool(handle)->bget(size);
}

void *bget_bgetz(bget_handle_t handle, bget_bufsize_t size) // alloc buffer from pool indicated by handle, and set 0
{
    return __bget_get_pool(handle)->bgetz(size);
}

void *bget_getr(bget_handle_t handle, void *buffer, bufsize newsize) //realloc buffer from pool indicated by handle
{
    return __bget_get_pool(handle)->bgetr(buffer, newsize);
}

void bget_brel(bget_handle_t handle, void *buf)
{
    return __bget_get_pool(handle)->brel(buf);
}

void bget_bstats(bget_handle_t handle, bufsize *curalloc, bufsize *totfree, bufsize *maxfree, long *nget, long *nrel)
{
    return __bget_get_pool(handle)->bstats(curalloc, totfree, maxfree, nget, nrel);
}

void bget_bpoold(bget_handle_t handle, void *pool, int dumpalloc, int dumpfree)
{
    return __bget_get_pool(handle)->bpoold(pool, dumpalloc, dumpfree);
}

int bget_test(bget_handle_t handle)
{
    return __bget_get_pool(handle)->test_main();
